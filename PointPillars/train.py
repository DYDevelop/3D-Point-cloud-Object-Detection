...
def main(args):
    ...
    if args.resume_from:
        ...
        # load trained scheduler state_dict
        scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
        print('Previously trained scheduler state_dict loaded...')
        pre_epochs = checkpoint['epoch']
        print(f"Previously trained for {pre_epochs} number of epochs...")
        # train for more epochs
        print(f"Train for {args.max_epoch - pre_epochs} more epochs...")

    for epoch in range(pre_epochs, args.max_epoch):
        print('=' * 20, epoch, '=' * 20)
        ...
        with torch.no_grad():
            for i, data_dict in enumerate(tqdm(val_dataloader)):
                ...
                val_step += 1
            print(f'Val Loss : {val_loss / len(val_dataloader):.3f}')
            if val_loss / len(val_dataloader) < best_loss:
                best_loss = val_loss / len(val_dataloader)
                torch.save({
                'model_state_dict': pointpillars.state_dict()
                }, os.path.join(saved_ckpt_path, f'best_ckpt.pth'))
        pointpillars.train()


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Configuration Parameters')
    parser.add_argument('--data_root', default='dataset/kitti', 
    ...
    # parser.add_argument('--resume-from', default='pillar_logs/checkpoints/epoch_140.pth')
